...
Another important aspect of the model definition is the inclusion of the random-effect parameters to account for inter individual variability (IIV) pf PK parameters. IIV describes variation between patients and is often added on clearance (`CL`) and/or volume of distribution (`V`). The typical value (TV) is the same for all patients, and a random value from an assumed distribution is added for each patient. For example: `CL = TVCL*exp(eta_CL)`, in which the `eta_CL` is a random value from a normal distribution. 

Similarly, covariate relationships can be added to PK parameters. The covariate weight, usually abbreviated as `WT`, is often added to CL, as many studies have shown that weight influence the clearance of drugs. You may specify this relationship in the code, such as: `CL = TVCL* (WT / WT_avg) ^ expo`.  The `WT` input comes from your data, as that is specific for each patient. The `WT_avg` is calculated in advance and you describe it in the model parameters secton. `expo` may be estimated but you will often see in models when `WT_avg` is 70 kg, `expo` is 0.75 according to allometric scaling. In this case, the `TVCL` describes the clearance value of a 70 kg adult. 
...
To provide the model simulation with an estimation for the width of the normal distribution, you need to provide values for `omega`. This is not as easy as simply providing a number for `eta_CL` and `eta_V1`, this has to be given in a matrix format, with the values diagonally. 
$$\begin{bmatrix}
0.25 & 0 \\
0 & 0.01 \\
\end{bmatrix}$$
